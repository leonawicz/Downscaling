		...
		if(method=="akima"){
			...
			for(h in 1:nlayers(b.anom.t)){
				...
				if(h==1){
					...
					xy <- coordinates(spTransform(xy, CRS=proj4.out))
				...
				if(exists("rc.ind")) { mat.t <- mat.t[rc.ind[1]:rc.ind[2], rc.ind[3]:rc.ind[4]]; mat.p <- mat.t[rc.ind[1]:rc.ind[2], rc.ind[3]:rc.ind[4]] }
				r.new.t <- raster(round(mat+mt.clim, 1), xmn=xmn, xmx=xmx, ymn=ymn, ymx=ymx, crs=proj4.out)
				r.new.p <- raster(round(mat*mp.clim), xmn=xmn, xmx=xmx, ymn=ymn, ymx=ymx, crs=proj4.out)
...
f(j=1, i=i, par.by.month=TRUE, anomDir=anomDir, outDir=outDir, b.clim.t=b.clim.t, b.clim.p=b.clim.p)
