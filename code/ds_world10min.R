##############################################################################################################################
#### This R script downscales GCM and CRU climate anomaly geotiffs to CRU 2.0 1961-1990 10-minute resolution climatology. ####
##############################################################################################################################
...
f <- function(j, i, par.by.month=FALSE, anomDir, outDir, b.clim.t, b.clim.p){
	...
	for(k in mo){
		...
		# Downscale anomalies and combine with CRU 2.0 climatology, write anual .tif files
		nc <- ncol(b.anom.t)
		for(h in 1:nlayers(b.anom.t)){
			r.anom.t <- subset(b.anom.t, h)
			r.anom.p <- subset(b.anom.p, h)
			if(h==1 & xmin(r.anom.t) < 0 & all(is.na(r.anom.t[nc]))) fill.last.col <- TRUE else fill.last.col <- FALSE # If TRUE, then TRUE for all h and both tas and pr
			if(fill.last.col){ # Last row is the same as first row
				r.anom.t[,nc] <- r.anom.t[,1]
				r.anom.p[,nc] <- r.anom.p[,1]
			}
			r.new.t <- round(mask(resample(r.anom.t, r.clim.t), r.clim.t) + r.clim.t, 1)
			r.new.p <- round(mask(resample(r.anom.p, r.clim.p), r.clim.p) * r.clim.p)
...
if(exists("month.index") && all(1:12 %in% month.index)){
	mclapply(month.index, f, i=i, par.by.month=TRUE, anomDir=anomDir, outDir=outDir, b.clim.t=b.clim.t, b.clim.p=b.clim.p, mc.cores=12)
} else mclapply(1:length(models), f, i=i, anomDir=anomDir, outDir=outDir, b.clim.t=b.clim.t, b.clim.p=b.clim.p, mc.cores=min(length(models), 32))
